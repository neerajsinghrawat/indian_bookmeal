        ...
        if(!empty($product_details)){
          ...
          if(isset($product_details->image) && !empty($product_details->image)){
            ...
            foreach ($product_details->productAttribute as $key => $productitem){ 
              ...
              $html .='<div class="col-md-6 form-group"><label class="custom-control custom-radio">              
               <input name="productAttribute[radio_'.$value.']" type="radio" class="custom-control-input attributes attributes_'.$value.'" value="'.$productitem->id.'" pricetype="'.$productitem->price_type.'" productAmount="'.$product_details->price.'" amount="'.$productitem->price.'" '.$checked.' feature="'.$value.'"><span class="custom-control-indicator"><svg class="icon" x="0px" y="0px" viewBox="0 0 32 32"><path stroke-dasharray="19.79 19.79" stroke-dashoffset="19.79" fill="none" stroke="#FFFFFF" stroke-width="4" stroke-linecap="square" stroke-miterlimit="10" d="M9,17l3.9,3.9c0.1,0.1,0.2,0.1,0.3,0L23,11"></path></svg></span> <span class="custom-control-description">'.$attribute_name.'('. getSiteCurrencyType().$tag.$productitem->price.')</span> </label></div>';
              } 
            } 
